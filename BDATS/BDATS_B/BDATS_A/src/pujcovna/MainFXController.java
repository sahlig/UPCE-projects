...
public class MainFXController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        btnOdstranVsude.setOnAction((event) -> {
           try {
                Auto klic = createKlic();
                Auto nasel = null;
                seznam.zpristupniPobocku(EnumPozice.PRVNI);
                obnovAuta();
                nasel = seznam.zpristupniPobocku(EnumPozice.AKTUALNI).hledejAuto(klic);
                while(nasel == null){
                        seznam.zpristupniPobocku(EnumPozice.NASLEDNIK);
                        obnovAuta();
                        nasel = seznam.zpristupniPobocku(EnumPozice.AKTUALNI).hledejAuto(klic);
                ...
                if(nasel != null){
                autoList.getSelectionModel().select(nasel);
                pujcList.getSelectionModel().select(seznam.zpristupniPobocku(EnumPozice.AKTUALNI));
                seznam.odeberAuto(nasel);
                obnovAuta();
                }
            } catch (Exception ex) {
                myAlert();
        ...
        btnNajit.setOnAction((event) -> {
            try {
                Auto klic = createKlic();
                autoList.getSelectionModel().select(seznam.zpristupniPobocku(EnumPozice.AKTUALNI).hledejAuto(klic));
            } catch (Exception ex) {
                myAlert();
            }
        });
        
        btnNajitVsude.setOnAction((event) -> {
            try {
                Auto klic = createKlic();
                Auto nasel = null;
                seznam.zpristupniPobocku(EnumPozice.PRVNI);
        ...
        generateIt.setOnAction((event) -> {
            try {
                ...
                seznam.zpristupniPobocku(EnumPozice.PRVNI);
                seznam.vlozAuto(new OsobniAuto("4H34728", 180000, 0, Barva.CERNA));
                seznam.vlozAuto(new OsobniAuto("5W12233", 180040, 0, Barva.BILA));
                seznam.vlozAuto(new OsobniAuto("9C46665", 1805600, 0, Barva.ZELENA));
                seznam.zpristupniPobocku(EnumPozice.NASLEDNIK);
                seznam.vlozAuto(new OsobniAuto("8S46666", 200000, 0, Barva.BILA));
                seznam.vlozAuto(new UzitkoveAuto("4S21321321", 1000, 0, 100));
                seznam.zpristupniPobocku(EnumPozice.NASLEDNIK);
                seznam.vlozAuto(new OsobniAuto("8L456123", 200333, 0, Barva.BILA));
                seznam.vlozAuto(new OsobniAuto("8L456dfg23", 2213, 0, Barva.CERNA));
                seznam.vlozAuto(new OsobniAuto("45sa6d4", 4563142, 0, Barva.BILA));
                seznam.vlozAuto(new OsobniAuto("as87d4s2f", 456785, 0, Barva.MODRA));
                seznam.zpristupniPobocku(EnumPozice.NASLEDNIK);
                seznam.vlozAuto(new OsobniAuto("5", 456785, 0, Barva.MODRA));
                seznam.vlozAuto(new OsobniAuto("3", 456785, 0, Barva.MODRA));
                seznam.vlozAuto(new UzitkoveAuto("7", 1000, 0, 100));
                seznam.vlozAuto(new UzitkoveAuto("1", 666, 0, 666));
                seznam.vlozAuto(new UzitkoveAuto("4", 1000, 0, 100));
                seznam.vlozAuto(new OsobniAuto("2", 456785, 0, Barva.MODRA));
                seznam.vlozAuto(new OsobniAuto("6", 456785, 0, Barva.MODRA));
                seznam.vlozAuto(new OsobniAuto("8", 456785, 0, Barva.MODRA));
                seznam.vlozAuto(new OsobniAuto("9", 456785, 0, Barva.MODRA));
                obnovAuta();
                obnovPujcovny();
            } catch (Exception ex) {
                myAlert();
    ...
    public void obnovPujcovny(){
        try {
            ...
            Iterator<Pobocka> ite = seznam.iterator(eTyp.POBOCKA, typIteratoru.getValue());
            while(ite.hasNext()){
    ...
    public void obnovAuta(){
        try {
            ...
            Iterator<Auto> ite = seznam.iterator(eTyp.AUTOMOBIL, typIteratoru.getValue());
            while(ite.hasNext()){
    ...
    public void obnovZapujcene(){
        try {
            ...
            Iterator<Auto> ite = seznam.iterator(eTyp.VYPUJCENY, typIteratoru.getValue());
            while(ite.hasNext()){
    ...
    private void createAuto(){
        ...
        if(result.isPresent()){
            if(result.get().equals("Osobní auto")){
                try {
                    new NewCar(seznam.zpristupniPobocku(EnumPozice.AKTUALNI), 0, this);
                } catch (Exception ex) {
                    myAlert();
                }
            }else if(result.get().equals("Užitkové auto")){
                try {
                    new NewCar(seznam.zpristupniPobocku(EnumPozice.AKTUALNI), 1, this);
                } catch (Exception ex) {
                    myAlert();
                }
            }
