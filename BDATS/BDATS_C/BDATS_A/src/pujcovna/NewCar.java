...
public class NewCar extends Application{
    ...
    IAbstrPriorQueue<Auto> tmpPob;
    int co;
    EnumPozice kde;
    MainFXController tmpPuj;

    public NewCar(IAbstrPriorQueue<Auto> jaka, int co, MainFXController pujcovna){
        tmpPob = jaka;
        this.co = co;
        this.kde = kde;
        tmpPuj = pujcovna;
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        btnOk.setOnAction((event) -> {
            ...
            if(co == 0){
                try {
                    tmpPob.vloz(new OsobniAuto(spz.getText(), Integer.parseInt(km.getText()), 0, barva.getValue()));
                    tmpPuj.obnovAuta();
                    primaryStage.close();
                } catch (Exception ex) {
                Alert a = new Alert(Alert.AlertType.ERROR);
                ...
                try {
                    tmpPob.vloz(new UzitkoveAuto(spz.getText(), Integer.parseInt(km.getText()), 0, Integer.parseInt(nosnost.getText())));
                    tmpPuj.obnovAuta();
                    primaryStage.close();
                } catch (Exception ex) {
                Alert a = new Alert(Alert.AlertType.ERROR);
