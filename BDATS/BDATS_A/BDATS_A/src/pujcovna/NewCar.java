...
public class NewCar extends Application{
    ...
    public NewCar(IPobocka jaka, int co, EnumPozice kde, MainFXController pujcovna){
        tmpPob = jaka;
        this.co = co;
        this.kde = kde;
        tmpPuj = pujcovna;
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        btnOk.setOnAction((event) -> {
            ...
            if(co == 0){
                try {
                    tmpPob.vlozAuto(new OsobniAuto(spz.getText(), Integer.parseInt(km.getText()), 0, barva.getValue()), kde);
                    tmpPuj.obnovAuta();
                    primaryStage.close();
                } catch (Exception ex) {
                Alert a = new Alert(Alert.AlertType.ERROR);
                ...
                try {
                    tmpPob.vlozAuto(new UzitkoveAuto(spz.getText(), Integer.parseInt(km.getText()), 0, Integer.parseInt(nosnost.getText())), kde);
                    tmpPuj.obnovAuta();
                    primaryStage.close();
                } catch (Exception ex) {
                Alert a = new Alert(Alert.AlertType.ERROR);
